	...
	render() {
		...
		return [
			...
			<div
				...
				className={ `${ className } position-relative offer-block overflow-hidden` }
			>
				<MediaUpload
					onSelect={ value => {
						setAttributes( { imgUrl: value.sizes.full.url } );
						this.setState( { bgImg: value.sizes.full.url } );
					} }
					render={ ( { open } ) => {
						return (
							<button
								...
								className="w-100 border-0 p-0 h-100 position-absolute"
								onClick={ open }>
								<div className="">
									{ attributes.imgUrl ?
										<img
				...
				<RichText
					...
					style={ 
						this.state.bgImg ? { color: '#ffffff' } : { color: '#303030' }
					}
				/>

				<RichText
					tagName="p"
					className="content__text mt-2"
				...
				<div className="d-flex align-items-center">
					<div className="col-2 pl-0 position-relative">
						<SelectControl
							className="mb-0"
